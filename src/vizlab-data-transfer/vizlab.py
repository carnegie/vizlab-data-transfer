...
def set_ip(ip):
    ...
    try:
        ipaddress.ip_address(ip) # use ipaddress module to check validity of given address
...
def set_port(port):
    ...
    try:
        if (int(port) < 0 or int(port) > 65535):
            print("The given port number is not valid.")
            pass
...
def _network_info_is_valid():
    ...
    if (not _ip_is_valid() or not _port_is_valid()):
        return False
    else:
        return True
...
def _ip_is_valid():
    try:
        ...
        print(f"The current IP address ({IP}) is not valid. Please use the 'set_ip' method to fix this!")
        return False

def _port_is_valid():
    try:
        if (int(PORT) < 0 or int(PORT) > 65535):
            print(f"The current port number ({PORT}) is not valid. Please use the 'set_port' method to fix this!")
            return False
        return True
    except:
        print(f"The current port number ({PORT}) is not valid. Please use the 'set_port' method to fix this!")
...
def _serialize_single_data(info, is_final_block):
    ...
    elif (isinatnce(info, astropy.fits.ImageHDU)):
        return _serialize_astropy_image_data(info, is_final_block)
    else:
        raise ValueError("Provided Python objects must be one of the following types: numpy ndarray or recarray, pandas dataframe, astropy FITS table, matplotlib figure, PIL image.")

def _serialize_header(size_in_bytes, dims, dtypes, name, unit, is_final_block):

    block_value = 1 if is_final_block else 0
    content_header = size_in_bytes.to_bytes(4, byteorder="little") + len(dims).to_bytes(4, byteorder="little") + len(dtypes).to_bytes(4, byteorder="little") + len(name).to_bytes(4, byteorder="little") + len(unit).to_bytes(4, byteorder="little") + (block_value).to_bytes(4, byteorder="little")
...
def _serialize_ndarray_data(arr, is_final_block=False):
    ...
    header = _serialize_header(arr.nbytes, arr.shape, [_get_internal_dtype(arr.dtype)], "dataset", "dimensionless", is_final_block)
...
def _serialize_recarray_data(arr, is_final_block=False):
    ...
    # Serialize header info
    header = _serialize_header(arr.nbytes, arr.shape+(len(arr.dtype.names),), _get_internal_dtypes(arr.dtype), _combine_string_list(arr.dtype.names), "dimensionless", is_final_block)
...
def _serialize_mpl_data(fig, is_final_block=False):
    ...
    # Serialize header info
    header = _serialize_header(buf.getbuffer().nbytes, [int(i) for i in list(fig.get_size_inches())]*300, [10], "dataset", "dimensionless", is_final_block)
...
def _serialize_pil_data(img, is_final_block=False):
    ...
    # Serialize header info
    header = _serialize_header(buf.getbuffer().nbytes, list(img.size), [10], "dataset", "dimensionless", is_final_block)
...
def _combine_string_list(lst):
    ...
    for i in range(len(lst)):
        ...
        combined_string += '\0' if i < len(lst) - 1 else "" # use null terminator to split in C#
...
def _get_internal_dtype(numpy_dtype):
    if (numpy_dtype is np.dtype(np.int8)):
        ...
        raise ValueError("Provided ndarray must be one of the following datatypes: np.int/uint(8, 16, 32, 64), np.float(16, 32, 64)")
